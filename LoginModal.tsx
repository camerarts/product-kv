...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onLoginSuccess, onGoogleLogin }) => {
  ...
  const handleLogin = async () => {
    ...
    try {
        ...
        if (res.ok) {
            // 验证成功
            onLoginSuccess(password); // Pass password back to App for memory storage
            resetForm();
            return;
        } else {
            // 验证失败
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-6" onClick={onClose}>
       <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-8 animate-scale-up text-center" onClick={e=>e.stopPropagation()}>
          ...
          <div className="space-y-4">
             ...
             {/* Admin Login (Username/Password) - Moved to Top */}
             ...
             <div className="relative py-2">
                ...
                <div className="relative flex justify-center text-xs">
                  <span className="px-2 bg-white text-neutral-400 font-medium">其他方式</span>
                </div>
             </div>
             ...
             <button 
                ...
                className="w-full py-3 bg-white border border-neutral-200 text-neutral-700 rounded-xl text-sm font-bold hover:bg-neutral-50 hover:border-neutral-300 transition-all flex items-center justify-center gap-3 shadow-sm group relative overflow-hidden"
             >
